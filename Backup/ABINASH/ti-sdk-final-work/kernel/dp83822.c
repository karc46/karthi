...
#if 0
	if ((prev_sspeed != phydev->speed) && (1 == lstatus))
	{
            if (SPEED_10 == phydev->speed)
	    {
		 if (0 == phydev->phyid)
		 {
			 printk("Kemsys PhyID 0 is 10 Mpbs \r\n");
		 }
		 else
		 {
			 printk("Kemsys PhyID 1 is 10 Mpbs \r\n");
		 }
	    }
	    else
            {
		 if (0 == phydev->phyid)
		 {
			 printk("Kemsys PhyID 0 is 100 Mpbs \r\n");
		 }
		 else
		 {
			 printk("Kemsys PhyID 1 is 100 Mpbs \r\n");
		 }
	    }
	    prev_sspeed = phydev->speed;
	}
#endif
#if 0
        //printk("phydev->mdio.addr = %x \r\n", phydev->mdio.addr);
        //printk("Status = %x DP83822_PHYSTS_LINK = %x \r\n", status, DP83822_PHYSTS_LINK);
	if (status & DP83822_PHYSTS_LINK)
	{
             //printk(" Link is up Phy%d \r\n", phydev->mdio.addr);
	     if ((0 == phydev->mdio.addr) && (prev_lstatus[phydev->mdio.addr] != 1))
	     {
		 printk("Kemsys Eth0 Link is up \r\n");
	     }
	     else
	     if (prev_lstatus[phydev->mdio.addr] != 1)
	     {
		 printk("Kemsys Eth1 Link is up \r\n");
	     }
	     prev_lstatus[phydev->mdio.addr] = 1;
	}
	else
	{
             //printk(" Link is Down Phy%d \r\n", phydev->mdio.addr);
	     if ((0 == phydev->mdio.addr) && (prev_lstatus[phydev->mdio.addr] != 0))
	     {
		 printk("Kemsys Eth0 Link is down \r\n");
	     }
	     else
	     if (prev_lstatus[phydev->mdio.addr] != 0)
	     {
		 printk("Kemsys Eth1 Link is down \r\n");
	     }
